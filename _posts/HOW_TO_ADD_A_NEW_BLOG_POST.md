...
6. Wait 1-2 minutes — GitHub automatically builds and publishes the page.
   No script to run, no bundle to rebuild, nothing else to upload. It will
   also show up automatically on your /blog/ listing page and in your
   sitemap.xml — both update themselves on every commit.

That's it. There is no separate blog data file, build script, or generator
to run for a new post — GitHub + Jekyll handle everything the moment you
commit a new file inside _posts/.

Tip: a ready-to-copy blank template also lives at /blog-post-template.md
in the repo root — duplicate it, rename it with today's date, drop it in
_posts/, fill in your text, and commit.
