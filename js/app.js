...
function App() {
  ...
  // Legacy hash-URL redirect: every page on this site now has its own real,
  // dedicated URL (no #/ routes). This only exists to send any old bookmarked
  // or shared #/... links to the correct real page.
  useEffect(() => {
    ...
    if (hash.startsWith('#/university/')) {
      ...
      window.location.replace(found ? `/universities/${found.slug}.html` : '/universities/index.html');
  ...
  // Real-URL navigation. Every "page" this links to is a real, dedicated,
  // crawlable URL — clicking any of these is a normal page load, not a
  // hash change, so the address bar always shows a proper URL.
  const navigateTo = (pageName, uni = null) => {
    if (pageName === 'university' && uni) {
      window.location.href = `/universities/${uni.slug}.html`;
